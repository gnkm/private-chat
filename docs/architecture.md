...
| 変更履歴 | 1.0: 初版作成 / 1.1: §5.2 リポジトリ・ディレクトリ構成を追加 / 1.2: §7.2・§12 に TBR-002（本文上限・NUL 禁止）を反映 / 1.3: §7.2 に投稿 JSON のフィールド名（`displayName`, `body`, `id`, `sentAt`）を確定記載 / 1.4: §7.3・§12 に新規接続時の履歴再送を行わない方針を確定記載 / 1.5: §7.3・§12 に再接続ポリシー（指数バックオフ・ベース待機上限 30 秒・ジッター 0〜500ms・手動再接続 UI なし）を確定記載 / 1.6: §7.3・§12 に接続状態の専用 UI を設けない方針を確定記載 |
...
- **サーバ → クライアント（メモリ保持・ブロードキャストの完成形）**: `id`（string。UUID 等の生成を推奨）、`displayName`（string）、`body`（string）、`sentAt`（string。**ISO 8601**。UTC かつミリ秒を含む形式を推奨。例: `2026-04-18T12:34:56.789Z`）。
...
- **新規 WebSocket 接続時は、メモリ上に保持している過去投稿を再送しない**（実装方針）。クライアントは接続後にサーバからブロードキャストされた投稿のみ一覧に反映する。SRS は接続前の履歴再送を要求しないため本方針は仕様と両立する。同一セッション内の **接続後に発生した投稿を全接続クライアントへ配信する** ことは必須である（SRS-FUNC-002）。
