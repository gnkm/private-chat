...
| 変更履歴 | 1.0: 初版作成 / 1.1: §5.2 リポジトリ・ディレクトリ構成を追加 / 1.2: §7.2・§12 に TBR-002（本文上限・NUL 禁止）を反映 / 1.3: §7.2 に投稿 JSON のフィールド名（`displayName`, `body`, `id`, `sentAt`）を確定記載 / 1.4: §7.3・§12 に新規接続時の履歴再送を行わない方針を確定記載 |
